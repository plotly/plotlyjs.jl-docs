...
jupyter:
  ...
  plotly:
    ...
    order: 36
    page_type: u-guide
    permalink: julia/figure-introspection/
    thumbnail: thumbnail/violin.jpg
---

### The Figure Lifecycle

As explained in the [Figure Data Structure documentation](/julia/figure-structure/), when building a figure object with Plotly.py, it is not necessary to populate every possible attribute. At render-time, figure objects are passed from PlotlyJS.jl to [Plotly.js](/javascript/), which is the Javascript library responsible for turning JSON descriptions of figures into graphical representations.
...
Figure objects created by PlotlyJS.jl have a number of attributes automatically set, and these can be introspected using the Julia `JSON.print()` function:

```julia
using PlotlyJS, JSON

fig = plot(scatter(x=[10, 20], y=[20, 10]), Layout(height=400, width=400))
...
<!-- TODO: I think this section doesn't apply to Julia? I think the Julia library already creates the full figure -->
...
fig = plot(
    ...
    Layout(height=400, width=400, template="none")
)
```
...
<!-- TODO: the rest of the docs are just about the `full_figure_for_development` output... -->
