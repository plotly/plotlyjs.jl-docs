...
A polar chart represents data along radial and angular axes. With Plotly Express, it is possible to represent polar data as scatter markers with `scatter_polar`, and as lines with `line_polar`.

The radial and angular coordinates are given with the `r` and `theta` arguments of `scatter_polar`. In the example below the `theta` data are categorical, but numerical data are possible too and the most common case.

```julia
using PlotlyJS, CSV, DataFrames
df = dataset(DataFrame, "wind")
plot(scatter_polar(df, r="frequency", theta="direction"))
```
